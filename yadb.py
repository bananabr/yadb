...
async def consumer(iqueue):
    while True:
        args = await iqueue.get()
        try:
            if not args:
                break
            message = await _resolve_hostname(**args)
...
async def main():
    ...
    parser.add_option("-w", "--workers", type=int, default=128, dest="workers",
